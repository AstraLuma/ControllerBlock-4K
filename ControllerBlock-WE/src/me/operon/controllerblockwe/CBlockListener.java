...
implements Runnable, Listener
{
	...
	public Player getPlayerEditing(CBlock c) {
		for (@SuppressWarnings("rawtypes") Map.Entry e : parent.map.entrySet()) {
	...
	public void onBlockBreak(BlockBreakEvent e) {
		...
		if ((player.getGameMode().equals(GameMode.CREATIVE)) && (item.isBlock()))
		{
			...
			if (conBlock == null)
			{
				...
				if (conBlock.numBlocks() == 0) {
					...
					if (((conBlock.protectedLevel == 1) || (conBlock.protectedLevel == 2)) && (!parent.isUnprotectedMaterial(item)))
					{
		...
		if ((conBlock != null) && (conBlock.hasBlock(b.getLocation())) && (conBlock.getType().equals(b.getType()))) {
			if (conBlock.delBlock(b)) {
				player.sendMessage("Block removed from controller " + Util.formatBlockCount(conBlock));
			}
		}
		else if ((conBlock = parent.getControllerBlockFor(null, b.getLocation(), b.getType(), null)) != null) {
			switch (((BlockProtectMode)parent.getConfigu().getOpt(Config.Option.BlockProtectMode)).ordinal()) {
			case 1:
	...
	public void onBlockDamage(BlockDamageEvent e)
	{
		...
		if ((e.isCancelled()) || (player.getGameMode().equals(GameMode.CREATIVE))) {
			return;
		}
		PlayerInventory inv = player.getInventory();
		Material item = inv.getItemInHand().getType();
	...
	public void onBlockPlace(BlockPlaceEvent e)
	{
		...
		if ((parent.getConfigu().getInt(Config.Option.MaxDistanceFromController).intValue() != 0) &&
				...
				(!parent.getPerm().isAdminPlayer(player)) &&
				(Util.getDistanceBetweenLocations(conBlock.getLoc(), e.getBlock().getLocation()) > parent.getConfigu().getInt(Config.Option.MaxDistanceFromController).intValue()))
		{
	...
	public void onBlockPhysics(BlockPhysicsEvent e) {
		...
		if (conBlock.isBeingEdited())
		{
			...
			if ((e.getBlock().getType().equals(Material.FENCE)) || (e.getBlock().getType().equals(Material.THIN_GLASS))) {
				return;
			}
			...
			BlockProtectMode protect = (BlockProtectMode)parent.getConfigu().getOpt(Config.Option.BlockPhysicsProtectMode);
	...
	public void onBlockFromTo(BlockFromToEvent e) {
		...
		if (conBlock.isBeingEdited())
		{
			...
			BlockProtectMode protect = (BlockProtectMode)parent.getConfigu().getOpt(Config.Option.BlockFlowProtectMode);
			if (protect.equals(BlockProtectMode.protect)) {
				e.setCancelled(true);
			} else if (protect.equals(BlockProtectMode.remove)) {
				conBlock.delBlock(e.getBlock());
	...
	public void onBlockPistonExtend(BlockPistonExtendEvent event)
	{
		if (((Boolean)parent.getConfigu().getOpt(Config.Option.PistonProtection)).booleanValue())
		{
			...
			for (int i = 0; i < pblocks.size(); i++)
			{
				...
				if ((!parent.isControlledBlock(block.getLocation(), block.getType())) || (parent.isUnprotectedMaterial(block.getType()))) {
					continue;
				}
				event.setCancelled(true);
			}
	...
	public void onBlockPistonRetract(BlockPistonRetractEvent event) {
		if (((Boolean)parent.getConfigu().getOpt(Config.Option.PistonProtection)).booleanValue())
		{
			...
			if (event.isSticky())
			{
				Block block = b.getWorld().getBlockAt(event.getRetractLocation());
			...
			for (@SuppressWarnings("rawtypes") Map.Entry e : parent.map.entrySet()) {
				...
				while (i.hasNext()) {
					...
					if (!Util.typeEquals(b.getType(), ((CBlock)e.getValue()).getType())) {
						parent.log.debug("Block at " + Util.formatLocation(b.getLocation()) + " was " + b.getType() + " but expected " + ((CBlock)e.getValue()).getType() + ", dupe!");
						i.remove();
